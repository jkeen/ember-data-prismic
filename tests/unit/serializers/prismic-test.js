...
module('Unit | Serializer | prismic', function(hooks) {
  setupTest(hooks);
  ...
  test('returns correct serializer for Post', function(assert) {
    assert.ok(this.owner.lookup('service:store').serializerFor('post') instanceof PrismicSerializer, 'serializer returned from serializerFor is an instance of PrismicSerializer');
  });
  ...
  test('normalize with empty resourceHash', function(assert) {
    ...
    assert.deepEqual(serializer.normalize(this.owner.lookup('service:store').modelFor('post'), resourceHash), { data: null });
  });
  ...
  test('normalize with single Post payload', async function(assert) {
    ...
    let normalizedPost = serializer.normalizeSingleResponse(this.owner.lookup('service:store'), this.owner.lookup('service:store').modelFor('post'), {"results": [singlePost]});
  ...
  test('author is an embedded relationship', function(assert) {
    ...
    let normalizedPost = serializer.normalizeResponse(this.owner.lookup('service:store'), this.owner.lookup('service:store').modelFor('post'), {results: [postResponse]}, 'development-has-started', 'findRecord');
  ...
  test('document linked within a slice should be included as relationship in all posts response', function(assert) {
    ...
    let serializer = this.owner.lookup('service:store').serializerFor('post');
    let normalizedPost = serializer.normalizeArrayResponse(this.owner.lookup('service:store'), this.owner.lookup('service:store').modelFor('post'), allPosts);
  ...
  test('document linked within a slice should be included as relationship', function(assert) {
    ...
    let normalizedPost = serializer.normalizeSingleResponse(this.owner.lookup('service:store'), this.owner.lookup('service:store').modelFor('post'), {results: [post]});
